...
OUT=/data/opentrng_campaign
OPENTRNG=../opentrng_ptrng
...
VIVADO="vivado"
AUTOMATION="${HARDWARE}/target/fpga/xilinx"
...
do
	...
	do
		...
		do
			...
			${VIVADO} -mode batch -source ${AUTOMATION}/generate_bitstream.tcl "${TARGET}/${PROJECT}.xpr" > "${OUT}/${SLUG0}_vivado.log"
			cp "${TARGET}/${PROJECT}.runs/impl_1/target.bit" "${OUT}/${SLUG0}_target.bit" >> "${OUT}/${SLUG0}_vivado.log"
			${VIVADO} -mode batch -source ${AUTOMATION}/program_bitstream.tcl -tclargs "${OUT}/${SLUG0}_target.bit" >> "${OUT}/${SLUG0}_vivado.log"
			...
			do
				...
				echo " - Reprogram FPGA"
				${VIVADO} -mode batch -source ${AUTOMATION}/program_bitstream.tcl -tclargs "${OUT}/${SLUG0}_target.bit" > "${OUT}/${SLUG1}_vivado.log"
